...
> **In the App Store:** agent-dashboard is listed in the official catalog, [kirodotdev/KiroCrewApps](https://github.com/kirodotdev/KiroCrewApps) (added in [#46](https://github.com/kirodotdev/KiroCrewApps/pull/46)), pinned to the `release` branch. Install it from the App Store tab: no clone, no Node and no uv on the target machine. The gateway installs the Python requirements into the app's own data dir on first start, and enabling the app registers the `agent-dashboard-troubleshooting` skill, so KiroCrew's diagnose chat knows the install paths, the backend log and the known failures.
